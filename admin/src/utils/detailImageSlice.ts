 ...
 * 但尺寸为 **790 × 21222px / 9.0MB**。这里必须分清三件事（2026-09-22 更正过一次口径，别再把它们混为一谈）：
 * - **文件大小** 9.0MB —— 只是压缩后的体积，**不等于**内存占用；
 * - **解码位图** `790 × 21222 × 4 ≈ 67MB` —— 这个量级手机给得起，**不是失败原因**；
 * - **单边像素** 高度 **21222px** —— 远超移动端单张位图的**边长上限**（常见 4096 / 8192），
 *   这才是真因：**解码阶段就直接失败**，小程序 `<image>` 只占位不显示（一片灰），
 *   `@error` 也未必触发（属于解码/渲染失败，而非加载失败）；
 *   另外按 `widthFix` 算出的渲染高度约 1 万 px，也会顶到页面 / 滚动区的限制。
 *
...
/**
 * 单段最大高度（px）：**3000**。
 * 依据：单段位图约 9.5MB（内存量级本身不致命），关键是**单边 3000px 落在所有机型的边长上限内**，
 * 任何手机都能解码；而整图 21222px 是**直接解码失败**；
